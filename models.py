...
#            logit(f"Exception posting track: {url}, {e}")
    ...
    def send_track(self, track, skip_active_check):
        ...
        doit = (skip_active_check or self._is_active()) and apikey and playlistkey and \
    ...
    def check_show_playlist(self, target_title):
        ...
        if not apikey or not playlist_apikey:
            tk.messagebox.showwarning("Missing API Key", "The user and playlist API keys must be set in order to use this feature. Contact your system administrator for help on obtaining these keys.")
            return False

        self.id = None
        title_safe = urllib.parse.quote(target_title)
        url = SystemConfig.playlist_host + f'/djtool/showplaylist/?show_title={title_safe}'
        logit(f"check playlist: {url}")
