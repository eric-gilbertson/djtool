...
class ZKPlaylist():
    ...
    def send_track_zookeeper(self, track, skip_active_check):
        ...
        doit = (skip_active_check or self._is_active()) and self.apikey and \
        ...
        url = SystemConfig.playlist_host + f'/api/v2/playlist/{self.id}/events'
    ...
    def send_track_proxy(self, track, skip_active_check):
        ...
        doit = (skip_active_check or self._is_active()) and self.apikey and self.proxy_apikey  and \
               not track.is_pause_file() and not track.is_spot_file()
        if not doit:
            logit(f"skip send_track {self.id}, {self._is_active()}")
            return
        ...
        req.add_header("Content-type", "application/vnd.api+json")
        req.add_header("Accept", "text/plain")
        req.add_header("X-APIKEY", self.proxy_apikey)
    ...
    def check_show_playlist(self, target_title, target_apikey, target_proxy_apikey):
        self.apikey = target_apikey
        self.proxy_apikey = target_proxy_apikey

        if SystemConfig.use_proxy:
            is_good = self.check_show_playlist_proxy(target_title, target_apikey, target_proxy_apikey)
        else:
            is_good = self.check_show_playlist_zookeeper(target_title, target_apikey)
    ...
    def check_show_playlist_zookeeper(self, target_title, apikey):
        self.id = None
        now_date = datetime.datetime.now().date().isoformat()
        url = SystemConfig.playlist_host + f'/api/v2/playlist?filter[date]={now_date}'

        try:
            logit(f"Check Zookeeper playlist {url}")
